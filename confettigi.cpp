...
QRectF ConfettiGI::boundingRect() const {
    qreal adjust = 0.5;
    return QRectF(-18 - adjust, -22 - adjust,
                  36 + adjust, 60 + adjust);
}

QPainterPath ConfettiGI::shape() const {
    QPainterPath path;
    path.addRect(-10, -20, 20, 40);
    return path;
}

void ConfettiGI::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *) {
    // Body
...
void ConfettiGI::advance(int step) {
    if (!step)
        return;
